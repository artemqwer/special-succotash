          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {!loading && !fetchError && (
              <div className="sm:hidden divide-y divide-gray-50">
                {filtered.length === 0 ? (
                  <p className="px-5 py-10 text-center text-[13px] text-gray-400">No users match your filters</p>
                ) : filtered.map(user => (
                  <div key={user.id} className="flex items-center gap-3 px-4 py-3.5 cursor-pointer active:bg-blue-50/30 transition" onClick={() => setEditUser(user)}>
                    {user.avatarUrl ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        ...
                        {user.revenueMonthly > 0 && (
                          <span className="text-[11px] font-medium text-gray-500">${user.revenueMonthly}/mo</span>
                        )}
                        {user.company && (
                          <span className="text-[11px] text-gray-400 truncate max-w-[120px]">{user.company}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex flex-col gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                      <button type="button" onClick={() => setEditUser(user)}
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition" title="Edit">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      </button>
                      <button type="button" onClick={() => setDeleteUser(user)}
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 transition" title="Delete">
