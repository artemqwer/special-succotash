...
export default function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const content = (
    <aside className={`h-full bg-white border-r border-gray-100 flex flex-col transition-all duration-200 ${collapsed ? "w-[60px]" : "w-[220px]"}`}>
      {/* Logo + collapse */}
      <div className="flex items-center justify-between px-4 pt-5 pb-3">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[9px] text-gray-400" style={{ letterSpacing: "0.03em" }}>E-commerce Intelligence</div>
            </div>
          </div>
        )}
        {collapsed && (
          <svg width="32" height="32" viewBox="0 0 42 42" fill="none" className="mx-auto">
            <rect width="42" height="42" rx="10" fill="#EFF4FF" />
            <rect x="7" y="26" width="6" height="10" rx="1.5" fill="#2563EB" />
            <rect x="15" y="19" width="6" height="17" rx="1.5" fill="#2563EB" />
            <rect x="23" y="11" width="6" height="25" rx="1.5" fill="#1D4ED8" />
        ...
        <button
          ...
          className="hidden lg:block text-gray-400 hover:text-gray-600 transition ml-auto"
        >
        ...
        {/* Mobile close button */}
        <button onClick={onMobileClose} className="lg:hidden text-gray-400 hover:text-gray-600 transition ml-auto">
      ...
      {/* User */}
      <div className={`border-t border-gray-100 p-3 flex items-center gap-2.5 ${collapsed ? "justify-center" : ""}`}>
        <div className="w-8 h-8 rounded-full bg-purple-500 flex items-center justify-center text-white text-[12px] font-bold shrink-0">JD</div>
        {!collapsed && (
          <div className="overflow-hidden">
            <p className="text-[13px] font-semibold text-gray-800 truncate">John Doe</p>
            <p className="text-[11px] text-gray-400 truncate">john@company.com</p>
          </div>
        )}
      </div>
    </aside>
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <div className="hidden lg:flex h-screen shrink-0">{content}</div>
